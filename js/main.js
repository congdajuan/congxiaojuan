...
(function (adz, $, WIN, DOC, undefined) {
    ...
    init = function () {
        ...
        if ($('html').hasClass('no-touch'))
            $('body#homeTpl nav a').on('click', scrollPage);
        ...
        $('a.anchor').on('click', function (e) {
            var offset = 0;
            e.preventDefault();
            if ($('body#homeTpl').length == 0)
                offset = ($('header').height() + 50) * - 1;
            $.scrollTo($(this).context.hash, 800, {
                offset: offset
            });
        });
    ...
    titleHeight = function () {
        ...
        if (winHeight >= 768 && $(WIN).width() > 615) {
            $('.title').each(function (e) {
                ...
                if (e == 0)
                    $(this).css({
                        'height': winHeight - (diff / 2) - headerHeight + 55,
                        'padding-top': (diff / 2) - 25
                    });
                else
                    $(this).css({
                        'height': winHeight - (diff / 2) - headerHeight ,
                        'padding-top': (diff / 2)
                    });
            });
        } else {
            ...
            $('.title:eq(0)').removeAttr('style').css({
                'padding': 0
            });
            ...
            $('.title:eq(0)').css({
                'height': winHeight - (diff / 2) - headerHeight + 30,
                'padding-top': (diff / 2) - 25
            });
        }
    };
    ...
    transition = function (e) {
        ...
        if (index != oldIndex) {
            ...
            $carousel.find('img').eq(oldIndex).css({
                'position': 'absolute'
            }).animate({
                'left': frameWidth
            });
            $carousel.find('img').eq(index).css({
                'left': - frameWidth,
                'display': 'block',
                'position': 'absolute'
            }).animate({
    ...
    spotTrigger = function () {
        ...
        if ( pos + 500 > $('#work').offset().top )
            startAnimation($spots.eq(0));
        if ( pos + 500 > $('#about').offset().top )
            startAnimation($spots.eq(1));
        if ( pos + 500 > $('#contact').offset().top )
            startAnimation($spots.eq(2));
        if ( pos >= 500 && $(WIN).width() > 615)
            navAnimation();
        if ( pos + 200 > $('.carousel').offset().top && !$('.carousel').hasClass('triggered')) {
            $('.carousel').addClass('triggered');
            setTimeout(function() {
                if ($('html').hasClass('no-touch'))
                    $pagination.find('li:eq(1) a').click();
            }, 3000);
        }
    };
    ...
    showMoreWork = function (e) {
        ...
        if ( shown < total ) {
            ...
            $wrapper.css({
                ...
                if ( shown + 2 >= total )
                    $btn.text(noMoreText).addClass('stop');
                else
                    $btn.text(nextText);
            });
        }
